  ...
  describe "#update" do
    ...
    describe "new agent" do
      ...
      context "receiving a RemoteAgent" do
        let(:agent) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 1, timestamp: 1, endpoints: [], rest_api: nil)}
    ...
    describe "old agent" do
      ...
      context "agent updated with correct information" do
        let(:agent_updated) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 2, timestamp: 2, endpoints: ["something"], rest_api: nil)}
      ...
      context "agent updating with the same information" do
        let(:agent) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 1, timestamp: 1, endpoints: ["something"], rest_api: nil)}
        let(:agent_updated) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 2, timestamp: 2, endpoints: ["something"], rest_api: nil)}
      ...
      context "agent updated with incorrect sequence number" do
        let(:agent_updated) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 1, timestamp: 2, endpoints: ["something"], rest_api: nil)}

        it "it does not update the info the sequence number is lower or equal than the previous one stored" do
          expect(subject).to_not receive(:correct_timestamp?)
          expect(list).to_not receive(:update).with(agent_uuid, agent_updated)
          subject.update(agent_updated)
        end
      end
      ...
      context "agent updated with nil information" do
        context "nil endpoints" do
          let(:agent) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 1, timestamp: 1, endpoints: ["something"], rest_api: nil)}
          let(:agent_updated) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 2, timestamp: 2, endpoints: [], rest_api: nil)}
        ...
        context "nil api" do
          let(:agent) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 1, timestamp: 1, endpoints: ["something"], rest_api: double("rest_api"))}
          let(:agent_updated) {instance_double(RemoteAgent, uuid: agent_uuid, sequence_number: 2, timestamp: 2, endpoints: ["something"], rest_api: nil)}
